...
export default function App() {
  ...
  const filtered = useMemo(() => leads.filter(l => {
    ...
    return true;
  }).sort((a,b) => b.analysis.score - a.analysis.score), [leads, campaignId, leadView, minimumScore, statusFilter, sourceFilter, scoreFilter, query]);
  ...
  return <main>
    ...
    <div className="wrap">
      ...
      {tab === "integrations" && <section><div className="notice"><Check size={18}/><div><strong>Uso pessoal e revisão humana</strong><p>A ferramenta organiza somente conteúdo público. Nenhuma mensagem é enviada e ninguém é incluído em grupos automaticamente.</p></div></div><div className="integration-grid"><Integration title="Busca pública" state="Ativa" text="HasData / Google SERP para localizar publicações públicas relevantes."/><Integration title="Comentários públicos" state="Beta" text="Reddit: leitura direta. Facebook: no máximo uma página pública por busca, somente comentários visíveis e com autor separado da página. Instagram/TikTok: tentativa em conteúdo visível sem login. Autoria incerta é descartada."/><Integration title="Importação manual" state="Ativa" text="Cole qualquer publicação ou comentário público encontrado e use o classificador V2."/></div></section>}
    </div>
    {selectedLead && <div className="modal-backdrop" onClick={()=>setSelectedLead(null)}><aside className="drawer" onClick={e=>e.stopPropagation()}><button className="close" onClick={()=>setSelectedLead(null)}>×</button><div className={scoreClass(selectedLead.analysis.score)}>{selectedLead.analysis.score}</div><h2>{selectedLead.analysis.band}</h2><p className="muted">{selectedLead.source} · {relativeDate(selectedLead.publishedAt)} · {isCommentLeadText(selectedLead.publicationText)?"💬 Comentário":"📝 Publicação"}</p><blockquote>{selectedLead.publicationText}</blockquote><h3>Por que foi classificado?</h3><ul>{selectedLead.analysis.signals.map(s=><li key={s}><Check size={15}/>{s}</li>)}</ul><div className="detail-grid"><div><span>Relevância</span><b>{selectedLead.analysis.relevance}%</b></div><div><span>Recência</span><b>{selectedLead.analysis.recencyWeight}%</b></div><div><span>Produto</span><b>{selectedLead.analysis.product??"—"}</b></div><div><span>Urgência</span><b>{selectedLead.analysis.urgency??"—"}</b></div></div><label>Situação<select value={selectedLead.status} onChange={e=>updateStatus(selectedLead.id,e.target.value as LeadStatus)}>{["Novo","Revisado","Contatado","Convertido","Descartado"].map(s=><option key={s}>{s}</option>)}</select></label>{selectedLead.publicationUrl!=="#"&&<a className="btn primary link" href={selectedLead.publicationUrl} target="_blank" rel="noreferrer">Abrir publicação <ExternalLink size={16}/></a>}</aside></div>}
  </main>
}
